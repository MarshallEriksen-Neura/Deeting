...
1. 前端请求 `GET /api/v1/auth/oauth/linuxdo/authorize`，服务端生成 `state` 并重定向到 LinuxDo 授权页面。
2. 用户完成授权后回到前端 `/callback` 页面，前端将 `code` 和 `state` POST 到 `/api/v1/auth/oauth/callback`。
