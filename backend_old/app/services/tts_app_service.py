...
class TTSAppService:
        ...
        try:
            ...
            for scored in ordered_candidates:
                ...
                try:
                    ...
                    if metadata and metadata.endpoints_config and "audio" in metadata.endpoints_config:
                        async for chunk in self._call_universal_tts_adapter(
                            ...
                            if cacheable and not cache_overflow:
                                if len(cache_buffer) + len(chunk) <= _MAX_CACHE_BYTES:
                                    ...
                                    cache_overflow = True
                            yield chunk
                    elif is_google_native_provider_base_url(str(cfg.base_url)):
    ...
    async def _call_universal_tts_adapter(
        self,
        *,
        provider_id: str,
        metadata: Any,
        ...
        """
        Calls a TTS provider using dynamic mapping rules from ProviderMetadata.
        """
        ...
        audio_cfg = metadata.endpoints_config["audio"]
        endpoint_path = audio_cfg.get("endpoint", "/v1/audio/speech")
        method = audio_cfg.get("method", "POST")
        input_map = audio_cfg.get("input_map")
        output_map = audio_cfg.get("output_map")
        response_maps = getattr(metadata, "response_maps", None)

        # 1. Transform Request
        transformer = RequestTransformer()
        # Inject processed text into a temporary copy of request if needed, 
        # but SpeechRequest already has 'input'.
        upstream_payload = transformer.transform(request, input_map)
        
        # Ensure model and input are correctly set
        if "model" in upstream_payload and not upstream_payload["model"]:
            upstream_payload["model"] = model_id
        if "input" in upstream_payload and not upstream_payload["input"]:
            upstream_payload["input"] = processed_text
        ...
        async with self.client.stream(method, url, headers=headers, json=upstream_payload) as resp:
            if int(getattr(resp, "status_code", 0) or 0) >= 400:
                ...
                raise HTTPException(
                    status_code=status.HTTP_503_SERVICE_UNAVAILABLE,
                    detail={
                        "message": f"upstream adapter HTTP {resp.status_code}",
                        "provider": provider_id,
                        "upstream_body": text[:800],
                    },
                )
            
            # 4. Handle Response (Direct stream or URL extraction)
            # If output_map has a url_extractor, it means it's not a raw audio stream
            if output_map and output_map.get("url_extractor"):
                body = await resp.aread()
                payload = json.loads(body.decode("utf-8"))
                adapter = ResponseAdapter()
                adapted = adapter.adapt_audio_response(payload, output_map, response_maps)
                if isinstance(adapted, dict) and "url" in adapted:
                    # If it's a URL, we need to fetch it
                    async with self.client.stream("GET", adapted["url"]) as audio_resp:
                        async for chunk in audio_resp.aiter_bytes(chunk_size=_CACHE_CHUNK_SIZE):
                            yield bytes(chunk)
                return

            # Default: stream raw bytes
