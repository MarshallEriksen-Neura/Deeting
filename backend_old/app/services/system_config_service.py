...
async def validate_kb_global_embedding_model_dimension(
    ...
    *,
    ...
    try:
        ...
        detail_obj = getattr(exc, "detail", None)
        upstream_status = _extract_upstream_status(detail_obj)
        if upstream_status is None and isinstance(detail_obj, dict):
            upstream_status = _extract_upstream_status(detail_obj.get("message") or detail_obj.get("error") or "")
        message = _extract_upstream_message(detail_obj)
        message = (message or "").strip()
        if not message or len(message) < 8:
            message = str(detail_obj or "")  # 回退到原始 detail 文本，避免丢失关键信息
