...
def _select_provider_endpoint(
    cfg: ProviderConfig, requested_style: str, capability: str = "chat"
) -> ProviderEndpointSelection | None:
    # Phase 4: Capability check (优先使用 capability 配置)
    has_explicit_cap = False
    if cfg.metadata and isinstance(cfg.metadata, dict):
        caps = cfg.metadata.get("capabilities")
        if isinstance(caps, dict) and capability in caps:
            has_explicit_cap = True

    if has_explicit_cap:
        cap = resolve_capability_config(
            provider_meta=cfg.metadata,
            capability=capability,
            base_url=str(cfg.base_url).rstrip("/"),
        )
        if cap:
            # Map adapter to style (临时兼容)
            style = "openai"
            if cap.adapter and "claude" in cap.adapter:
                style = "claude"
            elif "messages" in cap.endpoint and "anthropic" in cap.endpoint:
                style = "claude"
            return ProviderEndpointSelection(url=cap.endpoint, api_style=style)
...
async def _build_dynamic_logical_model_for_group(
    *,
    ...
    for cfg in providers:
        ...
        selection = _select_provider_endpoint(cfg, api_style, capability=capability)
