...
def _audio_object_prefix() -> str:
    # 使用独立的音频前缀，不复用图片前缀；允许可选的 image_oss_prefix 作为父级前缀，
    # 以便与图片存储目录保持一致（测试期望 object_key 以 image_oss_prefix 开头）。
    base = _normalize_prefix(getattr(settings, "image_oss_prefix", None))
    audio = _normalize_prefix(_AUDIO_OBJECT_KIND)
    if base and audio:
        return f"{base}/{audio}"
    if audio:
        return audio
