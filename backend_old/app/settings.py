...
class Settings(BaseSettings):
    ...
    embedding_models_require_input_type: str = Field(
        default="embed-english-v3,embed-multilingual-v3,cohere-embed,nemoretriever,nvidia/llama-3.2-nemoretriever",
        alias="EMBEDDING_MODELS_REQUIRE_INPUT_TYPE",
        description=(
            "逗号分隔的模型标识列表；当模型名包含其中任意子串时，网关会默认附加 input_type=search_document。"
            "用于非对称/双塔类 embedding 模型（如 Cohere/Nemo Retriever），避免逐个改代码。"
        ),
    )
