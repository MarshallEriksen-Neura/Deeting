...
def _has_explicit_chat_capability(cfg: Any) -> bool:
    if not cfg or not cfg.metadata or not isinstance(cfg.metadata, dict):
        return False
    caps = cfg.metadata.get("capabilities")
    return isinstance(caps, dict) and "chat" in caps
...
async def try_candidates_non_stream(
    *,
    ...
    for idx, upstream in enumerate(candidates):
        ...
        # Check for explicit capability config (Migration Path)
        cap_config = None
        if _has_explicit_chat_capability(provider_cfg):
        ...
        if cap_config:
            result = await execute_capability_transport(
                client=client,
                redis=redis,
                db=db,
                provider_id=provider_id,
...
async def try_candidates_stream(
    *,
    ...
    for idx, upstream in enumerate(candidates):
        ...
        # Check for explicit capability config
        cap_config = None
        if _has_explicit_chat_capability(provider_cfg):
            cap_config = resolve_capability_config(
                provider_meta=provider_cfg.metadata,
        ...
        if cap_config:
            iterator = execute_capability_stream(
                client=client,
                redis=redis,
                db=db,
                provider_id=provider_id,
