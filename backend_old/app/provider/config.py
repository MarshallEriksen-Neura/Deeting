...
def _convert_models(provider: Provider, preset=None) -> list[dict[str, Any]] | None:
    """
    ...
    if preset is not None and isinstance(getattr(preset, "static_models", None), list) and preset.static_models:
        cleaned: list[dict[str, Any]] = []
        for item in preset.static_models:
            if not isinstance(item, dict):
                continue
            copied = dict(item)
            if isinstance(copied.get("metadata"), dict):
...
def _build_provider_config(provider: Provider) -> ProviderConfig | None:
    preset = getattr(provider, "preset", None)
    ...
    base_url = _pick(provider.base_url, getattr(preset, "base_url", None))
    if not base_url:
        logger.warning("Provider %s skipped because base_url is missing (preset=%s)", provider.provider_id, getattr(preset, "preset_id", None))
        return None
    ...
    sdk_vendor = _pick(sdk_vendor, getattr(preset, "sdk_vendor", None))
    if sdk_vendor:
        data["sdk_vendor"] = sdk_vendor
    supported_styles = _pick(
        getattr(provider, "supported_api_styles", None),
        getattr(preset, "supported_api_styles", None),
    )
    ...
    headers = _convert_headers(
        _pick(provider.custom_headers, getattr(preset, "custom_headers", None))
    )
    if headers is not None:
        data["custom_headers"] = headers

    retryable = _convert_retryable_codes(
        _pick(provider.retryable_status_codes, getattr(preset, "retryable_status_codes", None))
    )
    if retryable is not None:
        data["retryable_status_codes"] = retryable

    static_models = _convert_models(provider, preset)
    if static_models is not None:
        data["static_models"] = static_models

    response_maps = _pick(getattr(provider, "response_maps", None), getattr(preset, "response_maps", None))
    if response_maps:
        data["response_maps"] = response_maps

    # Merge metadata from preset and provider (Provider overrides Preset)
    preset_meta = getattr(preset, "metadata_json", None) or {}
    provider_meta = getattr(provider, "metadata_json", None) or {}
    ...
    merged_meta = preset_meta.copy()
    merged_meta.update(provider_meta)
