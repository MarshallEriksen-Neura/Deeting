...
function normalizeBrowserToolName(value: string | null | undefined): string | null {
...
function describeToolCall(toolName: string): { phase: "waiting" | "acting" | "verifying" | "recovering"; label: string } {
  switch (toolName) {
    ...
    case "browser_retry_with_relocate":
      return { phase: "recovering", label: "Retrying after re-locating target" }
    default:
...
function applyBrowserToolResult(
  ...
  if (toolName === "browser_retry_with_relocate") {
    const retryCount = typeof result.attempts === "number" ? result.attempts : 0
...
export function useBrowserModeToolActivity(messages: Message[]) {
  ...
  useEffect(() => {
    ...
    if (block.type === "tool_result") {
      ...
      const toolArgs = relatedCall?.type === "tool_call" ? parseToolArgs(relatedCall.toolArgs) : null
      if (result) {
