...
function persistProcessedDeepLinkKey(processedKeys: Set<string>, key: string) {
  ...
  try {
    ...
    window.sessionStorage.setItem(PROCESSED_DEEP_LINK_STORAGE_KEY, JSON.stringify(nextValues));
  } catch {
...
export function DesktopOAuthListener() {
  ...
  useEffect(() => {
    ...
    const handleUrls = async (urls: string[]) => {
      for (const url of urls) {
        ...
        try {
          ...
          const message = getErrorMessage(error, "Desktop OAuth action failed");
    ...
    void (async () => {
      ...
      const message = getErrorMessage(error, "Desktop OAuth listener initialization failed");
      toast.error(message);
    });
