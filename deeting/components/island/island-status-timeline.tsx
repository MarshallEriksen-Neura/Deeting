...
import { ISLAND_STATUS_STEPS, type IslandStatusStep } from "./island-runtime-status";
...
export function IslandStatusTimeline({
  ...
  const detail = resolveStatusDetail(t, statusCode, statusMeta) ?? statusLabel;
  ...
  if (compact) {
    const compactLabel =
      (activeStage && steps.find((step) => step.key === activeStage)?.label) ?? statusLabel;
          ...
          return (
            <div key={step.key} className="flex items-start gap-2.5">
              ...
              <div className="min-w-0 flex-1">
                ...
                {isCurrent ? (
                  <p className="mt-0.5 text-[11px] leading-4 text-foreground/55">{detail}</p>
                ) : null}
              </div>
            </div>
          );
