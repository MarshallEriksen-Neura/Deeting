...
function areDerivedValuesEqual(left: unknown, right: unknown): boolean {
  if (Object.is(left, right)) return true;

  if (Array.isArray(left) && Array.isArray(right)) {
    if (left.length !== right.length) return false;
    return left.every((entry, index) => areDerivedValuesEqual(entry, right[index]));
  }
  ...
  if (isPlainObject(left) && isPlainObject(right)) {
    ...
    return leftKeys.every(
      (key) =>
        ...
        areDerivedValuesEqual(left[key], right[key])
    );
  }
...
export const useIslandStore = create<IslandState>((set) => ({
  ...
  hydrateFromChat: (snapshot) => {
    ...
    const statusLabel = deriveStatusLabel(snapshot, pendingApproval);
    const summaryText = deriveSummaryText(snapshot.messages);
    const lastReplyText = deriveLastReplyText(snapshot.messages);
    const lastReplyAt = latestAssistant?.createdAt ?? null;
    const recentMessages = deriveRecentMessages(snapshot.messages);
    ...
    set((state) => {
      ...
      if (
        state.statusLabel === nextHydratedState.statusLabel &&
        state.summaryText === nextHydratedState.summaryText &&
        state.lastReplyText === nextHydratedState.lastReplyText &&
        state.lastReplyAt === nextHydratedState.lastReplyAt &&
        state.errorMessage === nextHydratedState.errorMessage &&
        state.statusStage === nextHydratedState.statusStage &&
        state.statusCode === nextHydratedState.statusCode &&
        areDerivedValuesEqual(state.recentMessages, nextHydratedState.recentMessages) &&
        areDerivedValuesEqual(state.pendingApproval, nextHydratedState.pendingApproval) &&
        areDerivedValuesEqual(state.statusMeta, nextHydratedState.statusMeta) &&
        areDerivedValuesEqual(state.stageHistory, nextHydratedState.stageHistory)
      ) {
        return state;
      }

      return nextHydratedState;
    });
  },
