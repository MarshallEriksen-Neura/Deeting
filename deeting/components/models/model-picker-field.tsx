...
export function ModelPickerField({
  ...
  return (
    <div>
      ...
      >
        <PopoverTrigger asChild>
          <button
            ...
            className={cn(
              "flex min-h-11 w-full items-center justify-between rounded-xl border border-[var(--hairline)] bg-[var(--panel-bg-inset)] px-3 py-2 text-left transition-colors",
              "hover:border-[var(--hairline-strong)] hover:bg-[var(--panel-bg)] disabled:cursor-not-allowed disabled:opacity-60"
            )}
          >
            <span className="flex min-w-0 items-center gap-3">
              <span className="flex h-8 w-8 shrink-0 items-center justify-center rounded-xl bg-[var(--panel-bg)]">
        ...
        >
          {isOpen ? (
            <ModelPicker
              ...
              className="rounded-[1.5rem] border border-[var(--hairline)] bg-[var(--panel-bg)] shadow-[0_20px_60px_-30px_rgba(15,23,42,0.6)]"
            />
          ) : null}
        </PopoverContent>
      </Popover>
      ...
      {description ? (
        <p className="mt-1 text-[11px] text-[var(--ink-3)]">{description}</p>
      ) : null}
      {isUnlisted ? (
        <p className="mt-1 text-[11px] text-[var(--warn)]">{selectedValue}</p>
      ) : null}
    </div>
  )
}
