...
const ModelConfigPanel = dynamic(
  () => import("./model-config-panel").then((module) => module.ModelConfigPanel),
...
export const ModelAccordion = React.memo(function ModelAccordion({
  ...
  const handleOpenChange = React.useCallback((modelId: string, open: boolean) => {
  ...
  return (
    <div className="space-y-2">
      {models.map((model, index) => {
        ...
        return (
          <Collapsible key={model.id} open={isExpanded} onOpenChange={(open) => handleOpenChange(model.id, open)}>
            <ModelDataStrip
              model={model}
              index={index}
              onTest={onTest}
