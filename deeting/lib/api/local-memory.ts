...
export type LocalMemoryListResponse = z.infer<
  typeof LocalMemoryListResponseSchema
>;
...
export type LocalMemoryDeleteResponse = z.infer<
  typeof LocalMemoryDeleteResponseSchema
>;
...
export type LocalMemoryClearResponse = z.infer<
  typeof LocalMemoryClearResponseSchema
>;
...
export type LocalMemorySearchResult = z.infer<
  typeof LocalMemorySearchResultSchema
>;
...
export const WriteGuardResultSchema = z.object({
  ...
  top1_score: z.number().nullable().optional(),
  top2_score: z.number().nullable().optional(),
  score_gap: z.number().nullable().optional(),
  score_ratio: z.number().nullable().optional(),
  effective_update_threshold: z.number().nullable().optional(),
  effective_noop_threshold: z.number().nullable().optional(),
  protected_existing: z.boolean().nullable().optional(),
});
...
export async function appendLocalMemoryGuarded(
  ...
  ensureTauriRuntime();
  const data = await invokeTauri<WriteGuardResult>(
    "append_local_memory_guarded",
    {
      payload: {
        content: payload.content,
        session_id: payload.session_id ?? null,
...
export async function listLocalMemories(
  ...
  ensureTauriRuntime();
  const data = await invokeTauri<LocalMemoryListResponse>(
    "list_local_memories",
    {
      query: {
...
export async function searchLocalMemories(
  ...
  ensureTauriRuntime();
  const data = await invokeTauri<LocalMemorySearchResult>(
...
export async function deleteLocalMemory(
  id: string,
): Promise<LocalMemoryDeleteResponse> {
  ensureTauriRuntime();
  const data = await invokeTauri<LocalMemoryDeleteResponse>(
    "delete_local_memory",
    { id },
  );
...
export async function listMemorySnapshots(
  ...
  ensureTauriRuntime();
  const data = await invokeTauri<LocalMemorySnapshot[]>(
...
export async function clearLocalMemories(
  ...
  ensureTauriRuntime();
  const data = await invokeTauri<LocalMemoryClearResponse>(
...
export async function searchKnowledgeSemantic(
  ...
  ensureTauriRuntime();
  const data = await invokeTauri<KnowledgeSearchResult[]>(
