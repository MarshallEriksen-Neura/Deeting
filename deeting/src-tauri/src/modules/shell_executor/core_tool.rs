...
use super::{ShellExecutor, ShellExecutionRequest, create_shell_executor};
...
impl ShellExecuteCoreTool {
    ...
    pub async fn execute(&self, arguments: Value) -> Result<Value, String> {
        let request: ShellExecutionRequest = serde_json::from_value(arguments)
    ...
    /// 
    /// 用于集成到现有的风险评估系统
    pub fn assess_risk(&self, command: &str, arguments: &Value) -> crate::modules::mcp::ToolRiskAssessment {
        ...
        match policy {
            ...
            super::CommandPolicy::RequiresApproval(level, message) => {
                let (risk_level, boundary_class) = match level {
                    super::ApprovalLevel::Standard => ("MEDIUM", ApprovalBoundaryClass::SoftBoundary),
                    super::ApprovalLevel::Dangerous => ("HIGH", ApprovalBoundaryClass::HardBoundary),
                };
