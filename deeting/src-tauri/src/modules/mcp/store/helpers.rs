...
pub(super) fn row_to_local_knowledge_file(row: &SqliteRow) -> Result<LocalKnowledgeFile, McpError> {
...
pub(super) fn infer_file_type_from_filename(filename: &str) -> String {
...
pub(super) fn normalize_local_document_status(status: &str) -> &'static str {
...
pub(super) fn normalize_storage_document_status(status: Option<&str>) -> &'static str {
    match status
        .map(|value| value.trim().to_ascii_lowercase())
...
pub(super) fn truncate_local_document_error_message(raw: &str) -> String {
...
pub(super) fn extract_local_document_text(meta_info: &serde_json::Value) -> Option<String> {
...
pub(super) fn tokenize_local_search_query(query: &str) -> Vec<String> {
...
pub(super) fn compute_local_knowledge_match_score(
    query_lower: &str,
    tokens_lower: &[String],
    content_lower: &str,
) -> f64 {
...
pub(super) fn estimate_local_tokens(text: &str) -> i64 {
...
pub(super) fn sort_local_knowledge_folders(
...
pub(super) fn sort_local_knowledge_files(
...
pub(super) fn reverse_ordering(value: Ordering) -> Ordering {
    match value {
