pub(crate) fn extract_assistant_transition_blocks(
    ...
    let id_seed = if call_id.trim().is_empty() { tool_name } else { call_id };
    vec![serde_json::json!({
...
fn map_render_block_to_ui_block(
    ...
    let view_type = object
        ...
        .map(|value| value.trim())
        .filter(|value| !value.is_empty())?;
    let payload = object.get("payload").cloned().unwrap_or_else(|| serde_json::json!({}));
    let id_seed = if call_id.trim().is_empty() { tool_name } else { call_id };

    let mut block = serde_json::Map::new();
    block.insert("id".to_string(), serde_json::Value::String(format!("{id_seed}-ui-{index}")));
    block.insert("type".to_string(), serde_json::Value::String("ui".to_string()));
    block.insert("viewType".to_string(), serde_json::Value::String(view_type.to_string()));
    block.insert("payload".to_string(), payload);
    block.insert("displayMode".to_string(), serde_json::Value::String("widget".to_string()));
    ...
    if let Some(title) = object
        ...
        .filter(|value| !value.is_empty())
    {
        block.insert("title".to_string(), serde_json::Value::String(title.to_string()));
    }
    if let Some(metadata) = object.get("metadata").and_then(|value| value.as_object()) {
        block.insert("metadata".to_string(), serde_json::Value::Object(metadata.clone()));
    }
