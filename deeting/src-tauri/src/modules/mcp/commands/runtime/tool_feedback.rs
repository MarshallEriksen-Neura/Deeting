...
pub(crate) fn extract_chat_tool_calls(response: &serde_json::Value) -> Vec<LocalChatToolCall> {
    ...
    if let Some(tc_array) = response.get("tool_calls").and_then(|value| value.as_array()) {
        for tc in tc_array {
            let id = tc.get("id").and_then(|value| value.as_str()).map(|value| value.to_string());
            let name = tc
                .get("name")
                .and_then(|value| value.as_str())
                .or_else(|| tc.get("function").and_then(|f| f.get("name")).and_then(|value| value.as_str()))
                .map(|value| value.to_string());
            let args = tc
                .get("arguments")
                .cloned()
                .or_else(|| tc.get("function").and_then(|f| f.get("arguments")).map(|value| {
            ...
            if let Some(name) = name {
                calls.push(LocalChatToolCall { id, name, arguments: args });
            }
        }
    }
    calls
...
pub(crate) fn build_auto_code_mode_tool_feedback(
    ...
    let serialized = serde_json::to_string_pretty(&payload).unwrap_or_else(|_| {
        serde_json::json!({ "round": round, "results": results }).to_string()
    });
    let content = format!(
