...
use crate::modules::assistants::capability_registry::assistant_capability_availability;
#[cfg(test)]
...
#[cfg(test)]
fn db_mcp_tool_to_asset(tool: &McpTool) -> Value {
    json!({
        "id": tool.id,
        "name": tool.name,
...
impl RegistryAvailability {
    pub(crate) fn from_asset(
        ...
        match asset_type {
            ...
            "assistant" => {
                let availability = assistant_capability_availability(
                    asset_id,
                    asset_metadata,
                    read_path_mode,
                    enabled_assistant_ids,
                ...
                Self {
                    class: if availability.activation_required {
...
mod tests {
    ...
    fn db_mcp_tool_asset_preserves_transport_metadata() {
        let asset = db_mcp_tool_to_asset(&McpTool {
            id: "tool-1".to_string(),
