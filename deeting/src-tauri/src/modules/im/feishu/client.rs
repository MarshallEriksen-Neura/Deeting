...
impl FeishuClient {
            ...
            "event" => {
                ...
                match header.event_type.as_str() {
                    ...
                    "im.message.receive_v1" => {
                        ...
                        let im_event = convert_message_event(&event, header, frame.data.clone().unwrap_or(event_data.clone()));
                        
                        event_tx.send(im_event).await
                    ...
                    "card.action.trigger" | "card.action.trigger_v1" => {
                        ...
                        let im_event = convert_card_event(&event, header, frame.data.clone().unwrap_or(event_data.clone()));
    ...
    /// 回复卡片动作 API
    async fn reply_card_action_api(
        &self,
        token: &str,
        response: &Value,
    ) -> Result<(), ImError> {
...
impl ImClient for FeishuClient {
    ...
    async fn start(&self, event_tx: mpsc::Sender<ImEvent>) -> Result<(), ImError> {
        ...
        if self.config.app_id.is_empty() || self.config.app_secret.is_empty() {
            return Err(ImError::ConfigError("app_id 或 app_secret 未配置".to_string()));
        }
