...
mod tests {
    ...
    #[test]
    fn asset_score_key_prefers_stable_id_fields() {
        let key = asset_score_key(&json!({
            "id": "core.browser_open_tab",
            "name": "browser_open_tab"
        }))
        .expect("score key");

        assert_eq!(key, "core.browser_open_tab");
    }

    #[test]
    fn reciprocal_rank_fusion_rewards_items_present_in_multiple_rankings() {
        let bm25 = HashMap::from([
            ("browser_open_tab".to_string(), 1.0),
            ("shell_execute".to_string(), 0.6),
        ]);
        let structured = HashMap::from([
            ("browser_open_tab".to_string(), 0.7),
            ("browser_wait_for_element".to_string(), 1.0),
        ]);

        let fused = reciprocal_rank_fusion(&[&bm25, &structured]);

        let open_tab = fused.get("browser_open_tab").copied().expect("open tab");
        let shell = fused.get("shell_execute").copied().expect("shell");
        let wait = fused
            .get("browser_wait_for_element")
            .copied()
            .expect("wait");

        assert!(open_tab > shell);
        assert!(open_tab > wait);
    }
    ...
    fn normalize_score_map_scales_max_to_one() {
        ...
        assert_eq!(normalized.get("a").copied(), Some(1.0));
        assert_eq!(normalized.get("b").copied(), Some(0.5));
    }
}
