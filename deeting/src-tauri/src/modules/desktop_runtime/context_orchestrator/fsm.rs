...
pub fn render_context_manifest_prompt(manifest: &ContextManifest) -> Option<String> {
    ...
    if !manifest.available_tools.is_empty() {
        ...
        lines.push("Use context tools before relying on memory, wiki, or document evidence. Tool schemas and tool results carry detailed search strategy, coverage signals, recommended_next_action, and source_refs guidance.".to_string());
    }
