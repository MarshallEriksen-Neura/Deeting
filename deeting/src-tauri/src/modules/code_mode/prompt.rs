...
mod tests {
    ...
    fn render_execution_tool_prompt_includes_codemode_guidance() {
        ...
        assert!(prompt.contains("Saved HTML assets are recall references"));
    ...
    fn render_runtime_prompt_uses_discovery_as_truth_source() {
        ...
        assert!(prompt.contains("CLI or terminal workflows"));
        assert!(prompt.contains("`shell_execute`"));
        assert!(prompt.contains("no bespoke skill action name"));
        assert!(prompt.contains("Do not claim you cannot inspect the local machine"));
        assert!(prompt.contains("save it with `save_asset`"));
        assert!(prompt.contains("future reference material for recall"));
    }
    ...
    #[test]
    fn render_execution_tool_prompt_guides_cli_skill_docs_to_shell_execution() {
        let prompt = render_execution_tool_prompt(&[
            "search_sdk".to_string(),
            "shell_execute".to_string(),
            "execute_code_plan".to_string(),
        ...
        assert!(prompt.contains("`shell_execute`"));
        assert!(prompt.contains("loops, parsing, retries, or result aggregation"));
        assert!(prompt.contains("Do not require a dedicated skill action"));
    }
}
