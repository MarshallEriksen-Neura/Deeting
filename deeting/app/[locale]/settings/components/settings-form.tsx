...
export function SettingsForm({
  ...
  const [hasLoadedDesktopRelaySettings, setHasLoadedDesktopRelaySettings] =
  ...
  async function onSubmit(values: SettingsFormValues) {
    ...
    if (canEditDesktop && isTauriRuntime) {
      const { normalizeDesktopProxyMode } = await import("@/lib/api/desktop-config");
      const nextProxyMode = normalizeDesktopProxyMode(values.desktopProxyMode);
      if (nextProxyMode === "custom" && !values.desktopProxyUrl.trim()) {
        toast.error(t("toast.desktopProxyValidation"));
        return;
      }
    }
    setIsSaving(true);
        ...
        if (isTauriRuntime && hasLoadedDesktopRelaySettings) {
          ...
          try {
            ...
            if (
              ...
              await setDesktopNetworkProxySettings({
                mode: nextProxyMode,
                url: nextProxyUrl,
              });
            ...
            console.warn(
              "[desktop-settings] update relay settings failed",
              error,
            );
          }
        }
    ...
    <Form {...form}>
      <div className="flex flex-col gap-4 md:flex-row md:items-start md:gap-6">
        ...
        <form
          ...
          className="min-w-0 flex-1 space-y-5 md:space-y-6"
        >
          {/* Models section */}
          {activeSection === "models" && (
            <div className="flex flex-col gap-5 md:gap-6">
              ...
              {/* Rebuild banner */}
              {showRebuildBanner && (
                <div className="overflow-hidden rounded-[24px] border border-[var(--warn-border)] bg-[linear-gradient(145deg,color-mix(in_srgb,var(--warn-soft)_70%,white_30%)_0%,color-mix(in_srgb,var(--panel-bg)_88%,white_12%)_100%)] px-5 py-4 shadow-[0_18px_34px_-28px_rgba(196,131,18,0.45)]">
              ...
              {/* Repair section */}
              {canEditDesktop && (
                <div className="overflow-hidden rounded-[24px] border border-[var(--hairline-strong)] bg-[linear-gradient(145deg,color-mix(in_srgb,var(--panel-bg-inset)_88%,white_12%)_0%,color-mix(in_srgb,var(--panel-bg)_84%,white_16%)_100%)] px-5 py-4 shadow-[0_18px_34px_-28px_rgba(15,23,42,0.22)]">
                  <div className="flex items-start gap-3">
