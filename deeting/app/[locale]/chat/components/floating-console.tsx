...
export function FloatingConsole({
  ...
  const activeModelLabel =
    models.find((model) => model.id === selectedModelId)?.name ?? t("model.placeholder");
    ...
    >
              ...
              >
                <div className="space-y-4">
                  ...
                  <div className="text-xs text-muted-foreground/70">
                    {t("image.console.summary")}: {t("image.model")} {activeModelLabel} ·{" "}
                    {t("image.aspectRatio")} {ratioLabel} · {t("image.settings.steps")} {steps} ·{" "}
      ...
      >
        <span className="text-[11px] text-muted-foreground/50 tabular-nums">
          <KbdGroup>
            <Kbd className="bg-white/10 text-muted-foreground/70">Enter</Kbd>
            <span>{t("image.console.shortcuts.generate")}</span>
          </KbdGroup>
          <span className="mx-2 text-muted-foreground/30">|</span>
          <KbdGroup>
            <Kbd className="bg-white/10 text-muted-foreground/70">Shift + Enter</Kbd>
            <span>{t("image.console.shortcuts.newline")}</span>
          </KbdGroup>
        </span>
      </motion.div>
    </motion.div>
