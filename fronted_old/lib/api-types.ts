...
export interface MessageSpeechRequest {
  ...
  /** 直接提供参考音频 URL（若同时提供 prompt_audio_id，则以该字段为准） */
...
export type AgentTaskType = "crawl" | "extract" | "analyze";
export type AgentTaskStatus = "pending" | "queued" | "running" | "completed" | "failed" | "cancelled";
...
export interface AgentTaskLog {
  ...
  message: string;
  // Extra fields that might come from backend
  [key: string]: any;
}
...
export interface CreateCrawlTaskRequest {
  target_url: string;
  tags?: string[];
  task_type?: "crawl";
}
...
export interface RuleGenerateResponse {
  ...
  jmespath_rule: string;
  capability_rules: Array<Record<string, any>>;
