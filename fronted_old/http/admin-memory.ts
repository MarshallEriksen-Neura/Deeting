...
export const adminMemoryService = {
  ...
  listCandidates: async (
    ...
    const response = await httpClient.get(`/v1/admin/memories/candidates?${params.toString()}`);
  ...
  listPublished: async (
    ...
    const response = await httpClient.get(`/v1/admin/memories/published?${params.toString()}`);
    return response.data;
  },
  ...
  approve: async (
    ...
    const response = await httpClient.post(`/v1/admin/memories/${pointId}/approve`, data);
    return response.data;
  },
  ...
  create: async (data: AdminMemoryCreateRequest): Promise<AdminMemoryItem> => {
    const response = await httpClient.post('/v1/admin/memories', data);
    return response.data;
  },
  ...
  delete: async (pointId: string): Promise<void> => {
    await httpClient.delete(`/v1/admin/memories/${pointId}`);
  },
};
