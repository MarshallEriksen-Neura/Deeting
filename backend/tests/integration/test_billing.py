...
async def test_billing_integration_insufficient_balance(monkeypatch):
    ...
    with patch("app.services.workflow.steps.signature_verify.SignatureVerifyStep._verify_signature", 
        ...
        with patch("app.services.workflow.steps.upstream_call.UpstreamCallStep.execute") as mock_upstream:
            ...
            mock_upstream_result.status = "SUCCESS"
            # We need to set context variables that BillingStep uses
            async def mock_execute(ctx):
                ...
                ctx.upstream_result = MagicMock(provider="openai")
                return mock_upstream_result
            mock_upstream.side_effect = mock_execute
            
            # Mock BillingRepository.deduct to raise InsufficientBalanceError
            with patch("app.repositories.billing_repository.BillingRepository.deduct", 
                       AsyncMock(side_effect=InsufficientBalanceError(Decimal("30.0"), Decimal("0.5")))):
                
                async with AsyncClient(transport=ASGITransport(app=app), base_url="http://test") as ac:
                    headers = {"X-Api-Key": api_key, "X-Timestamp": "123", "X-Signature": "fake"}
