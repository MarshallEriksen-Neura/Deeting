...
class BanditRepository(BaseRepository[BanditArmState]):
    ...
    async def get_states_map(
        self, preset_item_ids: Iterable[str]
    ) -> dict[str, BanditArmState]:
        ids = list(preset_item_ids)
        if not ids:
            return {}
        ...
        if missing:
            stmt = select(BanditArmState).where(BanditArmState.preset_item_id.in_(missing))
            result = await self.session.execute(stmt)
            rows = result.scalars().all()
            for r in rows:
                try:
                    await cache.set_with_version(
                        CacheKeys.bandit_state(str(r.preset_item_id)),
                        self._serialize_state(r),
                        version if version is not None else 0,
                        ttl=self._cache_ttl,
                    )
                except Exception as exc:
                    logger.warning(f"bandit_cache_set_failed item={r.preset_item_id} exc={exc}")
                cached[str(r.preset_item_id)] = r
    ...
    async def ensure_state(
        ...
        """不存在则创建一个初始状态"""
        existing = await self.get_by_item(preset_item_id)
        if existing:
            return existing
        ...
        try:
            await cache.set_with_version(
                CacheKeys.bandit_state(preset_item_id),
                self._serialize_state(obj),
                version if version is not None else 0,
                ttl=self._cache_ttl,
            )
        except Exception as exc:
            logger.warning(f"bandit_cache_set_failed item={preset_item_id} exc={exc}")
        return obj

    async def get_by_item(self, preset_item_id: str) -> BanditArmState | None:
        stmt = select(BanditArmState).where(BanditArmState.preset_item_id == preset_item_id)
        result = await self.session.execute(stmt)
    ...
    async def get_report(
        ...
        stmt = (
            ...
            .join(ProviderPresetItem, BanditArmState.preset_item_id == ProviderPresetItem.id)
            .join(ProviderPreset, ProviderPresetItem.preset_id == ProviderPreset.id)
            .where(
                ProviderPreset.is_active == True,  # noqa: E712
                ProviderPresetItem.is_active == True,  # noqa: E712
            )
        )

        if capability:
            stmt = stmt.where(ProviderPresetItem.capability == capability)
        if model:
        ...
        if channel:
            if channel == "external":
                ...
                stmt = stmt.where(ProviderPresetItem.channel == channel)
        ...
        for row in rows:
            ...
            item: ProviderPresetItem = row.ProviderPresetItem
