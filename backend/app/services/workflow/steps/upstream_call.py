...
class UpstreamCallStep(BaseStep):
    ...
    async def execute(self, ctx: "WorkflowContext") -> StepResult:
        ...
        # 熔断保护
        if await self._is_circuit_open(upstream_url):
            ctx.mark_error(
        ...
        try:
            if is_stream:
                ...
                await self._mark_success(upstream_url)
                ...
                if response.get("raw_bytes"):
                    if len(response["raw_bytes"]) > settings.MAX_RESPONSE_BYTES:
                        ...
                        await self._mark_failure(upstream_url)
                        return StepResult(
                            status=StepStatus.FAILED,
    ...
    async def _is_circuit_open(self, url: str) -> bool:
        state = await self._get_cb_state(url)
        if state["state"] != "open":
            return False
        if time.time() - state["opened_at"] > settings.CIRCUIT_BREAKER_RESET_SECONDS:
            state["state"] = "half_open"
            state["success_count"] = 0
            await self._set_cb_state(url, state)
            return False
        return True

    async def _mark_failure(self, url: str) -> None:
        state = await self._get_cb_state(url)
        state["failures"] += 1
    ...
    async def _mark_success(self, url: str) -> None:
        state = await self._get_cb_state(url)
        if state["state"] == "half_open":
    ...
    async def _get_cb_state(self, url: str) -> dict[str, Any]:
        ...
        if not redis_client:
            return self._cb_state.setdefault(host, default.copy())

        key = f"{settings.CACHE_PREFIX}{CacheKeys.circuit_breaker(host)}"
        try:
            ...
            logger.warning(f"read circuit state failed host={host}: {exc}")
            return self._cb_state.setdefault(host, default.copy())

    async def _set_cb_state(self, url: str, state: dict[str, Any]) -> None:
        ...
        try:
            ...
            logger.warning(f"write circuit state failed host={host}: {exc}")
            self._cb_state[host] = state
