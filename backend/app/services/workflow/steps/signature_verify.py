    ...
    async def execute(self, ctx: "WorkflowContext") -> StepResult:
        ...
        if ctx.is_external and not all([timestamp, signature, api_key]):
            ...
            return StepResult(
                ...
                message="Missing timestamp, nonce, or signature",
            )
        ...
        try:
            ...
            ctx.mark_error(ErrorSource.GATEWAY, "SIGNATURE_INVALID", str(e))
            return StepResult(
                status=StepStatus.FAILED,
                message=str(e),
            )
