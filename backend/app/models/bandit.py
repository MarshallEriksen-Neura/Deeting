...
class BanditArmState(Base, UUIDPrimaryKeyMixin, TimestampMixin):
    ...
    preset_item_id: Mapped[uuid.UUID] = mapped_column(
        SA_UUID(as_uuid=True),
        ForeignKey("provider_preset_item.id", ondelete="CASCADE"),
        nullable=False,
        unique=True,
        index=True,
        comment="关联的路由条目",
    ...
    __table_args__ = (
        UniqueConstraint("preset_item_id", name="uq_bandit_arm_state_item"),
    )
